...
# --- 5. Train LightGBM Model with Threshold Optimization ---
def train_lgbm_model(X_train, y_train, X_test, y_test):
    ...
    plt.ylabel('F1 Score')
    plt.title('Threshold Optimization Based on F1 Score')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('f1_threshold_curve.png', dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('threshold_confusion_matrix_lgbm.png', dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    model, threshold = train_lgbm_model(X_train, y_train, X_test, y_test)
